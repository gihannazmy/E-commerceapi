...
def register(request):
    ...
    if user.is_valid():
        if not User.objects.filter(username=data['email']).exists():
            ...
            return Response({'details': 'Unable to craete '},status=status.HTTP_400_BAD_REQUEST)
    else:
...
def forgot_password(request):
    ...
    link = "http://localhost:8000/api/account/reset-password/{token}".format(token=token)
